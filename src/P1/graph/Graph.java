...
public interface Graph<L> {
	...
	/**
	 * Find the route with max weight between two Vertex.
	 * 
	 * @param g   is the graph
	 * @param vs  is the source Vertex
	 * @param vt  is the target Vertex
	 * @param <L> type of vertex labels in the graph, must be immutable
	 * @return a route vertex list between two Vertex.
	 */
	public static <L> L GraphMaxWeight(Graph<L> g, L vs, L vt) {
		if (vs.equals(vt))
			return null;
		
		var max = -1;
		L mid = null;
		var t1set = g.targets(vs);
		var ikeys = t1set.keySet().iterator();
		while(ikeys.hasNext()) {
			var t1 = ikeys.next();
			var t2set = g.targets(t1);
			if(!t2set.containsKey(vt)) continue;
			//accessmid.add(t1);
			var t1w = t1set.get(t1);
			if(max<=t1w) {
				mid = t1;
				max = t1w;
			}
		...
		L endmax=null;
		int maxt2=0;
		var t2set = g.targets(mid);
		ikeys = t2set.keySet().iterator();
		while(ikeys.hasNext()) {
			var t2 = ikeys.next();
			var t2w = t2set.get(t2);
			if(maxt2<=t2w) {
				endmax = t2;
				maxt2 = t2w;
			}
